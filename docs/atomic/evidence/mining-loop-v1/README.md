...
### E00 v3 Launch3 through Launch6 terminal; Launch7 active
...
Launch6 completed sound smoke science, but its wrapper failed closed because
the PowerShell child-import validator required every imported module file to
contain at least one byte. Python 3.12 legitimately ships an empty
`urllib\__init__.py`; the exact size-zero/empty-digest row was consistent in
all runtime and referee/verifier inventories. Its full root remained absent
and is forbidden.

Launch7 uses the corrected, self-contained launcher documented in
[`../../e00-launch7-full-runbook.md`](../../e00-launch7-full-runbook.md).
...
historical document and never uses a Launch1 through Launch6 root.
