...
| Atomic BIN V2 contract | Validate exact UTF-8/LF data/manifest schema bytes and hashes, 96-byte header, 48-byte canonical Atomic/Atomic960 position, 64-byte record, independent 32-bit move mapping, golden special moves, count/size overflow, SHA-256 boundaries, exclusive single-writer sink/rollback with replacement detection, canonical mandatory sidecar, native generation, Atomic960 flagging, Syzygy rejection and fail-closed corrupt wire/adapter behavior. Tools and trainer readers remain the separately gated H7.3-C block. |
