...
if ($bindings.Count -ne 1) {
    throw "Launch3 native bundle must contain exactly one pyffish binding"
}
...
) 84 168 'atomic-e00-src-full-v3-20260725' 'full schedule'
...
) 1 2 'atomic-e00-src-smoke-v3-20260725' 'smoke schedule'
...
if ($exitCode -ne 0) {
    throw "full runner exited $exitCode; Launch3 full is terminal"
}
if (
    -not (Test-Path -LiteralPath $FullStderrPath -PathType Leaf) -or
    (Get-Item -LiteralPath $FullStderrPath).Length -ne 0
) {
    throw "full runner stderr is not byte-empty; Launch3 full is terminal"
}
