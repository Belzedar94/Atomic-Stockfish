...
//
// The base engine short-circuited every non-NORMAL move to a flat zero. That
// is the one class where a flat zero is a lie: en passant and capturing
// promotions detonate like any other capture, and the search asks see_ge()
// with a POSITIVE threshold exactly when the capture history has already gone
// very negative -- so a permanent failure there handed the decision to a
// history that had already learned to distrust those moves. Answering with the
// real delta gives material its vote back. Castling and quiet promotions keep
// the neutral answer, which blast_see() returns for them by construction.
