...
1. start two single-use `UciEngine` instances with identical options except
   `EvalFile`;
2. record advertised options and engine IDs, acknowledge `isready`, send
   `ucinewgame`, and acknowledge `isready` again before starting clocks;
...
`atomic-e00-owned-process-v1` evidence. An accepted natural completion requires
direct-child exit zero, `terminate_job_called=false`, and
...
“E00 v2 post-commit runbook”. Any command-line, hash, schema or timeout change
requires a new design root and a fresh audit.
