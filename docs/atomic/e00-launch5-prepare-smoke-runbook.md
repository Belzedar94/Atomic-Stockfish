# E00 source v3 Launch5 prepare-and-smoke runbook

This is the only supported entrypoint for creating the fresh Launch5 design
...
partial creation makes that exact Launch5 root terminal.
...
- smoke seed: `atomic-e00-src-smoke-v3-launch5-20260725`;
- full seed: `atomic-e00-src-full-v3-launch5-20260725`;
...
Launch5 freezes both corrected Windows serialization boundaries:
...
if ($LASTEXITCODE -ne 0) {
    throw 'Launch5 prepare/smoke validation failed; do not execute'
}
```
...
& powershell.exe -NoLogo -NoProfile -NonInteractive `
  ...
  -ExpectedSourceTree $Tree
if ($LASTEXITCODE -ne 0) {
    throw 'Launch5 prepare/smoke is terminal; do not retry or recover its roots'
}
```
...
receipt consumed by
[`e00-launch5-full-runbook.md`](e00-launch5-full-runbook.md). The full launcher
