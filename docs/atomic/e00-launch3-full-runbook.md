# E00 source v3 Launch3 full runbook

This is the only supported entrypoint for the fresh Launch3 full battery:
...
or objects left in an earlier PowerShell process. Launch1 and Launch2, and any
...
- experiment `atomic-e00-src-v3-20260725`;
- smoke battery `atomic-e00-src-v3-launch3-smoke`;
- full battery `atomic-e00-src-v3-launch3-full`;
...
```json
{"audit":{"p0":0,"p1":0,"verdict":"GO"},"experiment_id":"atomic-e00-src-v3-20260725","full":{"battery_id":"atomic-e00-src-v3-launch3-full","runtime_build_receipt_sha256":"<lowercase sha256>","schedule_receipt_sha256":"<lowercase sha256>"},"schema":"atomic-e00-full-authorization-v1","smoke":{"battery_id":"atomic-e00-src-v3-launch3-smoke","execution_receipt_sha256":"<lowercase sha256>"},"source":{"commit":"<lowercase git commit>","tree":"<lowercase git tree>"}}
```
...
if ($LASTEXITCODE -ne 0) {
    throw 'Launch3 full validation failed; do not execute'
}
```
...
& powershell.exe -NoLogo -NoProfile -NonInteractive `
  ...
  -AuditReceiptPath $Audit
if ($LASTEXITCODE -ne 0) {
    throw 'Launch3 full is terminal; do not retry or recover this root'
}
```
...
failure makes this Launch3 full root terminal. Freeze it for forensics and use
