# E00 source v3 Launch7 full runbook

This is the only supported entrypoint for the fresh Launch7 full battery:
...
or objects left in an earlier PowerShell process. Launch1 through Launch6, and
any partial Launch7 root, are terminal evidence: never execute, recover,
collect, analyse, edit, delete or reuse them.
...
```json
{"audit":{"p0":0,"p1":0,"verdict":"GO"},"experiment_id":"atomic-e00-src-v3-launch7-20260725","full":{"battery_id":"atomic-e00-src-v3-launch7-full","runtime_build_receipt_sha256":"<lowercase sha256>","schedule_receipt_sha256":"<lowercase sha256>"},"schema":"atomic-e00-full-authorization-v1","smoke":{"battery_id":"atomic-e00-src-v3-launch7-smoke","execution_receipt_sha256":"<lowercase sha256>"},"source":{"commit":"<lowercase git commit>","tree":"<lowercase git tree>"}}
```
...
if ($LASTEXITCODE -ne 0) {
    throw 'Launch7 full validation failed; do not execute'
}
```
...
`.pyo` artifacts in the relevant source packages. The dedicated Launch7 capture
...
& powershell.exe -NoLogo -NoProfile -NonInteractive `
  ...
  -AuditReceiptPath $Audit
if ($LASTEXITCODE -ne 0) {
    throw 'Launch7 full is terminal; do not retry or recover this root'
}
```
...
failure makes this Launch7 full root terminal. Freeze it for forensics and use
