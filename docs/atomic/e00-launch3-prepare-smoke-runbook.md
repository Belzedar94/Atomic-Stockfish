# E00 source v3 Launch3 prepare-and-smoke runbook

This is the only supported entrypoint for creating the fresh Launch3 design
...
partial creation makes that exact Launch3 root terminal.
...
- smoke seed: `atomic-e00-src-smoke-v3-20260725`;
- full seed: `atomic-e00-src-full-v3-20260725`;
...
if ($LASTEXITCODE -ne 0) {
    throw 'Launch3 prepare/smoke validation failed; do not execute'
}
```
...
& powershell.exe -NoLogo -NoProfile -NonInteractive `
  ...
  -ExpectedSourceTree $Tree
if ($LASTEXITCODE -ne 0) {
    throw 'Launch3 prepare/smoke is terminal; do not retry or recover its roots'
}
```
...
receipt consumed by
[`e00-launch3-full-runbook.md`](e00-launch3-full-runbook.md). The full launcher
