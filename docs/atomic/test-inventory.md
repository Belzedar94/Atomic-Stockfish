...
| AtomicNNUEV3 private execution | H9.3h keeps V3 outside the production dispatcher and executes the strict mixed-wire fixture only through a scalar full-refresh oracle. C++ and an independent Python wire decoder compare both perspectives' feature rows, canonical i32 accumulators, i64 PSQT, transform bytes, all dense intermediates and final scales across 102 frozen positions plus targeted rows/buckets. Gates include all eight buckets, Atomic960, EP, signed truncation, adversarial dense boundaries, transactional failures, 1/2/4/8 readers, byte-exact save, three forced layouts, debug/AVX2/BMI2, sanitizers/Valgrind and continued V3 dispatcher rejection. No Elo/OpenBench applies until V3 can affect a playing move. |
