# E00 source v3 Launch4 prepare-and-smoke runbook

This is the only supported entrypoint for creating the fresh Launch4 design
...
Launch2 or Launch3. Those launches are terminal forensic evidence and must never be
...
partial creation makes that exact Launch4 root terminal.
...
- smoke seed: `atomic-e00-src-smoke-v3-launch4-20260725`;
- full seed: `atomic-e00-src-full-v3-launch4-20260725`;
...
Launch4 also freezes the corrected Windows PowerShell 5.1 array boundary:
...
if ($LASTEXITCODE -ne 0) {
    throw 'Launch4 prepare/smoke validation failed; do not execute'
}
```
...
& powershell.exe -NoLogo -NoProfile -NonInteractive `
  ...
  -ExpectedSourceTree $Tree
if ($LASTEXITCODE -ne 0) {
    throw 'Launch4 prepare/smoke is terminal; do not retry or recover its roots'
}
```
...
receipt consumed by
[`e00-launch4-full-runbook.md`](e00-launch4-full-runbook.md). The full launcher
