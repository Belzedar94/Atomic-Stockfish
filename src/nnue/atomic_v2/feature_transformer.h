...
#ifndef ATOMIC_V2_FEATURE_TRANSFORMER_H_INCLUDED
#define ATOMIC_V2_FEATURE_TRANSFORMER_H_INCLUDED
...
class FeatureTransformer {
   ...
   public:
    ...
    static constexpr u32 get_hash_value() { return FeatureHash ^ (OutputDimensions * 2); }
    ...
    // Read network parameters. Each compressed field is checked without
    // assertions and must contain exactly the declared canonical SLEB bytes.
    bool read_parameters(std::istream& stream) {
        if (!IO::read_signed_leb(stream, biases) || !IO::read_signed_leb(stream, weights)
            || !IO::read_signed_leb(stream, psqtWeights))
            return false;
        permute_weights();
        return bool(stream);
    }
    ...
    bool write_parameters(std::ostream& stream) const {
        ...
        return IO::write_signed_leb(stream, copy->biases)
            && IO::write_signed_leb(stream, copy->weights)
            && IO::write_signed_leb(stream, copy->psqtWeights);
    }
    ...
    alignas(CacheLineSize) std::array<WeightType, HalfDimensions * FeatureSet::Dimensions> weights;
    alignas(
      CacheLineSize) std::array<PSQTWeightType, FeatureSet::Dimensions * PSQTBuckets> psqtWeights;
};

static_assert(FeatureTransformer::InputDimensions == 45056);
static_assert(FeatureTransformer::OutputDimensions == 1024);
static_assert(FeatureTransformer::get_hash_value() == FeatureTransformerHash);

}  // namespace Stockfish::Eval::NNUE::AtomicV2

template<>
struct std::hash<Stockfish::Eval::NNUE::AtomicV2::FeatureTransformer> {
    Stockfish::usize
    operator()(const Stockfish::Eval::NNUE::AtomicV2::FeatureTransformer& ft) const noexcept {
        return ft.get_content_hash();
    }
};

#endif  // ATOMIC_V2_FEATURE_TRANSFORMER_H_INCLUDED
