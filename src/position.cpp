...
// The move ordering does not want the raw delta but the delta measured against
// what the move actually INVESTS. blast_see() charges the spent capturer at
// AtomicCapturePieceValue, so a passed pawn on the 7th is spent as if it were
// any pawn while the piece it would have promoted to is worth nothing -- it
// dies inside its own blast. Surcharge the investment instead of pretending the
// two pawns are the same pawn.
...
// greater or equal to the given threshold. Non-NORMAL moves keep the flat zero
// of the base engine: extending them is a change of policy that belongs to its
// own test, never a passenger of this one.
